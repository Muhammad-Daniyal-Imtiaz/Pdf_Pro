...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Add footer text to the last page if not present
    const lastPage = pages[pages.length - 1]
    if (lastPage && !lastPage.elements.some(el => el.content?.includes('Generated by'))) {
      lastPage.elements.push({
        id: 'footer-' + Date.now(),
        type: 'paragraph',
        x: 60,
        y: height - 80,
        content: 'Generated by Gemini AI & MCP Services - Professional PDF Generation System 2026 PDF Pro',
        style: {
          width: width - 120,
          height: 40,
          fontSize: 10,
